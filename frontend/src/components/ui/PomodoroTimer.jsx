...
const PomodoroTimer = () => {
  ...
  // Load saved position & collapsed state
  useEffect(() => {
  ...
  const handleMouseDown = (e) => {
    ...
    if (
      ...
      e.target.closest(`.${styles.settings}`) ||
      e.target.closest(`.${styles.collapseBtn}`) ||
      e.target.closest(`.${styles.alertBox}`)
    )
      return;
    setIsDragging(true);
    dragRef.current = {
    ...
    <>
      ...
      >
        ...
        <div className={styles.controls}>
          {!isRunning ? (
            <button onClick={() => setIsRunning(true)} className={styles.btn}>
              ▶️
            </button>
          ) : (
            <button onClick={() => setIsRunning(false)} className={styles.btn}>
              ⏸️
            </button>
          )}
