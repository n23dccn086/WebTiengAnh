...
const SRSDaily = () => {
  ...
  const handleLearnMore = async () => {
    ...
    try {
      ...
      if (result.added_count > 0) {
        alert(`Đã thêm ${result.added_count} từ mới vào danh sách ôn tập hôm nay!`);
        await loadCards();
        setFinished(false);
        setCurrentIndex(0);
        setFlipped(false);
      } else {
        alert('Bạn đã học hết từ mới trong các bộ thẻ đã bật SRS.');
      }
    } catch (err) {
      console.error(err);
      setError(err.response?.data?.message || 'Không thể thêm từ mới');
    } finally {
  ...
  if (finished || cards.length === 0) {
    return (
      <div className={styles.empty}>
        ...
        <button onClick={handleLearnMore} disabled={learningMore} className={styles.learnMoreBtn}>
          {learningMore ? 'Đang thêm...' : '📖 Học thêm từ mới'}
        </button>
  ...
  return (
    <div className={styles.container}>
      ...
      <FlipCard card={current} isFlipped={flipped} onFlip={() => setFlipped(!flipped)} />
