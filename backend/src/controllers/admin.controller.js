...
const updateServiceStatus = catchAsync(async (req, res) => {
  ...
  return successResponse(res, "Cập nhật trạng thái danh mục thành công", {
    id,
    status,
  });
});
...
const createSystemFlashcardSet = catchAsync(async (req, res) => {
  ...
  const data = await AdminService.createSystemFlashcardSet(
    adminId,
    service_id,
    title,
    description,
...
const importSystemFlashcardSet = catchAsync(async (req, res) => {
  if (!req.file)
    throw new AppError(400, "Vui lòng upload file Excel/CSV", "MISSING_FILE");
  const { title, description, service_id } = req.body;
  if (!title || !service_id)
    throw new AppError(
      400,
      "Thiếu tên bộ thẻ hoặc service_id",
      "MISSING_FIELDS",
    );
