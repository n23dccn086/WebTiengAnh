...
// ✅ Sửa đúng cách require pdf-parse
...
// 1. HÀM TRÍCH XUẤT PDF (có fallback Groq)
const extractVocabFromPdf = async (fileBuffer) => {
  try {
    const model = genAI.getGenerativeModel({
      model: "gemini-2.5-flash", // Giữ nguyên 2.5
      generationConfig,
      safetySettings
    });
...
Trả về MẢNG JSON thuần chứa các object có key: word, meaning, pronunciation, example_sentence, part_of_speech.`;
    ...
    if (error.message.includes("429") || error.message.includes("quota") || error.message.includes("fetch failed")) {
      console.warn("⚠️ [GEMINI LỖI PDF]: Đang chuyển sang GROQ API...");
      try {
        if (!process.env.GROQ_API_KEY) throw new Error("Thiếu GROQ_API_KEY trong file .env");
        // ✅ Dùng pdfParse đã require đúng
        const pdfData = await pdfParse(fileBuffer);
        const extractedText = pdfData.text;
        
        const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });
        const groqPrompt = `Bạn là chuyên gia ngôn ngữ. Hãy trích xuất TỐI ĐA 50 từ vựng tiếng Anh có giá trị học thuật từ đoạn văn bản sau. BỎ QUA các từ thông dụng như: the, a, is, are, and, or, in, on, at.
Trả về MẢNG JSON thuần chứa các object có key: word, meaning, pronunciation, example_sentence, part_of_speech. KHÔNG DÙNG MARKDOWN.
...
${extractedText}`;
        ...
        console.error("❌ [LỖI FALLBACK GROQ] (PDF):", groqError.message);
        throw new AppError(500, "Hệ thống AI đang quá tải, không thể xử lý file PDF. Vui lòng thử lại.", "AI_ALL_FAILED");
      }
    }
...
// 2. HÀM TẠO CÂU HỎI (giữ nguyên fallback)
